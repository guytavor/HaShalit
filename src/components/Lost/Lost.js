    ...
    return (
        <div className={styles.wrapper} onClick={startOver}>
            <div className={styles.content}>
                ...
                <h1 className={styles.title}>{content.lose.subTitle.replace('${years}', score)}</h1>
                {score > highScore ?
                    <h2 className={styles.highScore}>{content.lose.highScore}</h2> :
                    <h2 className={styles.highScore}>{content.lose.oldHighScore.replace('${highScore}', highScore)}</h2>}
